...
class SWCClient:
    ...
    This SDK class simplifies the process of using the SWC Fantasy Football
    API. It supports all the funtions of the WSC API and returns validated
    data types.
    ...
    def call_api(
        ...
        try:
            with httpx.Client(base_url=self.swc_base_url) as client:
                logger.debug(
                    f"""base_url: {self.swc_base_url}, api_endpoint: {api_endpoint},
    ...
    def get_health_check(self):
        """"Checks if API running and healthy.
        
        Calls the API health check endopoint and returns a standard mesagge
        if the API is running normally. Can be used to check status of API
        befor making more complicate API calls
        
        Returns:
        An httpx.Response object that contains the HTTP status, JSON response
        an other information received from the API.
        """
    ...
    def list_leagues(
        ...
        Calls the API v0/leagues endpoint an treturns a list of League 
        objects.
    ...
    def get_player_by_id(self, player_id: int) -> Player:
        ...
        response = self.call_api(endpoint_url)
        response_player = Player(**response.json())
    ...
    def list_performance(
        ...
        logger.debug("Entered list performance")
        params = {
            "skip": skip,
            "limit": limit,
            "minimum_last_changed_date": minimum_last_changed_date,
    ...
    def get_counts(self) -> Counts:
        ...
        response_counts = Counts(**response.json())
    ...
    def get_bulk_performance_file(self) -> bytes:
        """Return a bulk file with performance data of players."""
    ...
    def get_bulk_team_file(self) -> bytes:
        """Return a bulk file with teams data."""
    ...
    def get_bulk_team_player_file(self) -> bytes:
        """Return a bulk file with team players data"""
        logger.debug("Entered get bulk team file")
