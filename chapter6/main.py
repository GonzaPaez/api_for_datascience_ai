...
api_description = """
This API provide read-only access to info from the SportsWorldCentral (SWC) Fantasy Football API.
...
## Scoring
Yoou can get a list of NFL player performances, incluiding the fantasy points they scored using SWC league scoring.
...
def read_players(
    ...
    minimum_last_changed_date: date = Query(
        None, description="The minimum date of change that you want to return records. Exclud any records changed before this."
    ), 
...
@app.get(
    ...
    description="If you have an SWC Player ID of a player from another API call such as v0_get_players, you can call this API usin the player ID",
